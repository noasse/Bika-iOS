...
- 可选的私人云端历史同步，iOS 与 macOS 共用，本地配置自建 HTTPS 服务端，并由 VPS 上的 SQLite 历史库保存共享记录
...
云端历史同步默认关闭，仓库不会保存服务器地址、Token 或证书 pin。用户可以在 iOS/macOS 设置页本地填写自建 HTTPS 服务端、Bearer Token 和证书 SHA-256 pin 后启用。配套 VPS 服务会把共享历史记录写入 SQLite 历史库，只保留最新 200 条，并且只暴露 App 需要的 HTTPS API。
